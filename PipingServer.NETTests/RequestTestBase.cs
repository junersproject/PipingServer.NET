...
namespace Piping.Tests
{
    public class RequestTestBase
    {
        ...
        protected async Task<(HttpResponseHeaders, string BodyText)> GetVersionAsync(Uri BaseUri) => await GetResponseAsync(new Uri(BaseUri.ToString().TrimEnd('/') + "/version"), HttpMethod.Get);
        /// <summary>
        /// 
        /// </summary>
        ...
        /// <returns></returns>
        protected async Task<(HttpResponseHeaders Headers, string BodyText)> GetResponseAsync(Uri SendUri, HttpMethod Method, CancellationToken Token = default)
        {
